...
# Local test harness for the connector's function-calling loop.
# Gemini Enterprise itself calls the connector directly via the OpenAPI spec (see openapi.yaml);
# this script is only for exercising get_databricks_schema/execute_databricks_sql locally.
load_dotenv()
...
def function_calls_from(response):
    calls = []
...
while (function_calls := function_calls_from(response)):
    tool_parts = []
